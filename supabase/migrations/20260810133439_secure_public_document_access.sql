-- Remove the token-blind public policies: they matched ANY row whose
-- public_token was merely NOT NULL, exposing every shared invoice/quote
-- (and their lines) to the anonymous API key, and allowed anonymous updates.
DROP POLICY IF EXISTS "public read invoice by token" ON public.invoices;
DROP POLICY IF EXISTS "public update invoice signature by token" ON public.invoices;
DROP POLICY IF EXISTS "public read quote by token" ON public.quotes;
DROP POLICY IF EXISTS "public update quote signature by token" ON public.quotes;
DROP POLICY IF EXISTS "public read invoice_lines by token" ON public.invoice_lines;
DROP POLICY IF EXISTS "public read quote_lines by token" ON public.quote_lines;
...
-- Public share links are now served by a definer function that compares the
-- caller's token to the row's token exactly.
...
BEGIN
  ...
  WHERE (q.public_token IS NOT NULL AND q.public_token = p_token)
     OR (v_uuid IS NOT NULL AND q.id = v_uuid)
  LIMIT 1;
  ...
  LEFT JOIN public.profiles pr ON pr.id = co.user_id
  WHERE (i.public_token IS NOT NULL AND i.public_token = p_token)
     OR (v_uuid IS NOT NULL AND i.id = v_uuid)
  LIMIT 1;
...
-- Signing is a single-use state transition: the function refuses a document
-- that is already signed and only ever writes status + signature_data.
...
REVOKE ALL ON FUNCTION public.sign_public_document(text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.sign_public_document(text, text, text, text) TO anon, authenticated;
